...
def compute_loss_and_accuracy(
    ...
    with torch.no_grad():
        for (X_batch, Y_batch) in dataloader:
            ...
            prediction = utils.to_cuda(torch.tensor([torch.argmax(prob) for prob in output_probs]))

            # Compute Loss and Accuracy
            average_loss += loss_criterion(output_probs, Y_batch).cpu().item()

            accuracy += (prediction == Y_batch).sum().cpu().item()

            num_images += prediction.shape[0]
            iterations += 1

    accuracy = accuracy / num_images
    average_loss = average_loss / iterations
