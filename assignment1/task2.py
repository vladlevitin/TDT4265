...
def calculate_accuracy(X: np.ndarray, targets: np.ndarray, model: BinaryModel) -> float:
    ...
    error = targets - (Y_hat < 0.5).astype(int)

    accuracy = np.abs(error).sum() / targets.size
...
class LogisticTrainer(BaseTrainer):
    ...
    def train_step(self, X_batch: np.ndarray, Y_batch: np.ndarray):
        ...
        Y_hat = self.model.forward(X= X_batch)
        self.model.backward(X=X_batch, outputs=Y_hat, targets=Y_batch)

        self.model.w = self.model.w - self.learning_rate * self.model.grad

        loss = cross_entropy_loss(targets= Y_batch, outputs= Y_hat)
