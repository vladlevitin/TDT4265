...
def pre_process_images(X: np.ndarray):
    ...
    X = (1 - (-1))/(np.max(X) - np.min(X)) * (X - np.max(X)) - 1
...
def cross_entropy_loss(targets: np.ndarray, outputs: np.ndarray) -> float:
    ...
    loss = np.sum(-(targets * np.log(outputs) + (1 - targets) * np.log(1 - outputs))) / targets.size
...
class BinaryModel:
    ...
    def backward(self, X: np.ndarray, outputs: np.ndarray, targets: np.ndarray) -> None:
        ...
        self.grad = - np.matmul(X.T, (targets - outputs))/ targets.size
